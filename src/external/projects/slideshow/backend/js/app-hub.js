...
angular
    ...
    .constant('TEMPLATES_ROOT', 'https://www.fredericaerts.com/external/projects/slideshow/backend/templates')
    .constant('IMAGES_ROOT', 'https://www.fredericaerts.com/external/projects/slideshow/backend/images')
    .constant('DATA_ROOT', 'https://www.fredericaerts.com/external/projects/slideshow/backend/data');
...
angular
    ...
    .config(function($stateProvider, $urlRouterProvider) {
      ...
      $stateProvider
        .state('presentation', {
          url: '/',
          templateUrl: 'partials/content/presentation.html',
        ...
        .state('presentation-full', {
          url: '/full-screen',
          templateUrl: 'partials/content/presentation--full.html',
          controller: ['$rootScope',
            function ($rootScope) {
              $rootScope.pageName = 'Content';
            }
...
angular
    .module('app')
    .controller('ShellCtrl', ['$scope', '$element', '$rootScope', '$state', '$timeout', '$mdMedia', '$mdSidenav', '$mdUtil', '$compile',
  function($scope, $element, $rootScope, $state, $timeout, $mdMedia, $mdSidenav, $mdUtil, $compile) {
    'use strict';
    ...
    vm.sideNavLeft = {
      ...
      items: [
        ...
        },
        {
          name: 'Content',
          state: 'presentation',
          activeStates:['content','content-edit','liveSession','newSession', 'presentation'],
          icon: 'flare',
          subItems: [
            // {
            //   name: 'Articles',
        ...
        },
        {
          name: 'Audience',
          state: 'presentation',
          activeStates: ['audience', 'user profile'],
          icon: 'group',
          subNavHeight: 0,
          subItems: [],
          collapsed: true
        ...
        },
        {
          name: 'Pages',
          state: 'presentation',
          activeStates:['pages', 'pagesTemplatesArticle'],
          icon: 'view_quilt',
          subNavHeight: 0,
          subItems: [],
          collapsed: true
      ...
      buttons: [
        {
          ...
          menu: {
            ...
            items: [
              {
                title: '1 new VIP has signed up for Pega Webinar',
                clickHandler: function() {}
              },
              {
                title: '3 VIP\'s downloaded a report',
                clickHandler: function() {}
              },
              {
                title: '15 new hub accounts created',
                clickHandler: function() {}
              },
              {
                title: '2 articles published',
                clickHandler: function() {}
              }
            ],
            footer: {
            ...
            items: [
              {
                ...
                title: 'Steven McMillan, Accenture',
                text: 'Please send the report to me when it\'s published',
                clickHandler: function() {}
              },
              {
                image: 'profile_picture.jpg',
    ...
    /*  EVENT LISTENERS
        =========================================== */
    var maxConversationsCount = setMaxConversationsCount();
    angular.element(window).bind('resize', handleWindowResize);

    $rootScope.$on('openMessageWindow', function($event, conversation) {
        var numberOfConversations = vm.conversations.filter(function(conversation) {
          return conversation;
        }).length;

        if (vm.conversations.indexOf(conversation) > -1 || numberOfConversations >= maxConversationsCount) {
          return;
        }
        else {
          vm.conversations.push(conversation);
          var conversationIndex = vm.conversations.length - 1;
          var conversationWindow = $compile('<message-window ' +
                                    'ng-class="{\'minimized\': messageWindow.windowMinimized}" ' +
                                    'ng-if="shell.conversations[' + conversationIndex + ']" ' +
                                    'conversation="shell.conversations[' + conversationIndex + ']">' +
                                    '</message-window>')($scope);
          $element.parent().append(conversationWindow);
        }
    });

    $rootScope.$on('closeMessageWindow', function($event, conversation) {
        vm.conversations[vm.conversations.indexOf(conversation)] = undefined;
    });
    ...
    function selectZone(selectedHubZoneName) {
      if(!selectedHubZoneName) {
        vm.appBar.hubZones.selected = '';
        return;
      }
      for (var i = 0; i < vm.appBar.hubZones.names.length; i++) {
        if (vm.appBar.hubZones.names[i] === selectedHubZoneName) {
          vm.appBar.hubZones.selected = selectedHubZoneName;
          break;
        }
      }
    }

    function setMaxConversationsCount() {
      if($mdMedia('gt-md')) { // has sidebar
        maxConversationsCount = Math.floor((window.innerWidth - 300) / 360);
      }
      else {
        maxConversationsCount = Math.floor((window.innerWidth - 40) / 360);
      }
      maxConversationsCount = maxConversationsCount > 8 ? 8 : maxConversationsCount;
      maxConversationsCount = maxConversationsCount < 1 ? 1 : maxConversationsCount;
      return maxConversationsCount;
    }

    function handleWindowResize() {
      setMaxConversationsCount();
      $rootScope.sidenavLocked = $mdMedia('gt-md') && vm.sideNavLeft.isOpen;
    }
  }
]);
