...
var transcent = transcent || {};

transcent.nav = (function($, window, undefined) {

    var init, setContent, setNavblocks, setOnclickNavLinks, setOnclickMobileToggle, setMenuOpen;

    var menuOpen = false;

    init = function() {
        setOnclickNavLinks($('.js-nav-link'));
        setOnclickMobileToggle();
    };

    setOnclickNavLinks = function(elements) {
        elements.click(function(event) {
            var navDestination = $(this).attr("href");

            setContent('header', navDestination);
            setContent('main', navDestination);

            setNavblocks(navDestination);

            $('.navbar .js-nav-link').removeClass('navbar__link--active');
            $(".navbar .js-nav-link[href='" + navDestination + "']").addClass('navbar__link--active');

            $('.navbar-toggle--close').removeClass('navbar-toggle--close');
            menuOpen = false;
    ...
    setOnclickMobileToggle = function() {
        $('.js-toggle-navbar').click(function() {
            if(menuOpen) {
                $(this).removeClass('navbar-toggle--close');
                menuOpen = false;
            }
            else {
                $(this).addClass('navbar-toggle--close');
                menuOpen = true;
            }
        });
    };

    setContent = function(section, page) {
        $.ajax({
            // url: "http://localhost:8080/web/templates/" + section + "/_" + page + ".html",
            url: "http://localhost:8080/web/templates/" + section + "/_" + page + ".html",
            dataType: "html"
        })
        .done(function( data ) {
            ...
            if(page === "home") {
                $('#particles-js').empty();
                transcent.particles.init();
                setOnclickNavLinks( $('.js-btn-link--main') );
            }
    ...
    setNavblocks = function( page ) {
        var currentInvisibleBlock = $('.js-nav-blocks-container').attr('data-invisible-navblock');
        ...
        $.ajax({
            // url: "http://localhost:8080/web/templates/submain/navblocks/_" + currentInvisibleBlock + ".html",
            url: "http://localhost:8080/web/templates/submain/navblocks/_" + currentInvisibleBlock + ".html",
            dataType: "html"
        })
        .done(function( data ) {
            $(".js-nav-block[href='" + page + "']").replaceWith(data);

            setOnclickNavLinks( $(".js-nav-block[href='" + currentInvisibleBlock + "']") );

            $('.js-nav-blocks-container').attr('data-invisible-navblock', page );
        });
    };
...
transcent.particles = (function($, window, undefined) {
...
transcent.app = (function($, window, undefined) {

    var init, appResize, initOnload, debounce;
    ...
    appResize = function() {
        var _onResizeNavbar;
        var _onResizeParticles;

        _onResizeNavbar = debounce(function() {
            var $navbarToggleClose = $('.navbar-toggle--close'); 
            if( $navbarToggleClose ) {
                $navbarToggleClose.removeClass('navbar-toggle--close');
                transcent.nav.setMenuOpen(false);
            }
        }, 50);

        _onResizeParticles = debounce(function() {
            $('#particles-js').empty();
            transcent.particles.init();
        }, 1000);
